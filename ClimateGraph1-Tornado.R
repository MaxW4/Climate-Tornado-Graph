...
getwd()
setwd("/Users/maxwang/Desktop/ClimateStory/dataset+code")
...
t_data %>%
  ...
  geom_label(data = grid_labels, aes(x=x, y=y, label = labels), inherit.aes = FALSE, fill = "black", color = "gold", label.size = 0, size = 3) +
  geom_vline(xintercept = c(-5,-4, 0, 1), color = "gold") + 
  geom_segment(size = 0.9, lineend = "round") + 
  geom_text(data = year_labels, aes(x=x, y=y, label=y), inherit.aes= FALSE, color = "gold", size = 3, fontface = "bold") +
  scale_color_gradient2(low = "darkblue", high = "darkred", mid = "white", midpoint = 0, guide = "none") +
...
ggsave("/Users/maxwang/Desktop/ClimateStory/figures/ClimateGraph1.png", width = 4.5, height = 3.5, units = "in") 
